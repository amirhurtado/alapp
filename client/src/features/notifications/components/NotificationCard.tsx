...
const NotificationCard = ({ notification }: NotificationCardProps) => {
  return (
    <Link
      ...
      className="border-border border-1 rounded-lg p-4 bg-hover flex flex-col"
    >
      <div className="flex gap-3 items-center">
        <Avatar src={notification.sender.imageUrl} />
        <div className="flex items-center gap-2">
          {notification.type === "like" ? (
            <Heart size={18}  className="text-icon-pink"/>
          ) : notification.type === "favorite" ? (
            <Star  size={18}  className="text-icon-yellow"/>
          ) : notification.type === "repost" ?  (
            <Repeat2  size={18}  className="text-icon-green"/>
           ) : notification.type === "commment" ?  <MessageSquare  size={18}  className="text-primary-color"/>
           : <h1></h1>
